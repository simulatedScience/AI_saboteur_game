...
class SaboteurGUI:
    """
    GUI class that renders the game board and the current player's hand.
    ...
    def __init__(self, env: SaboteurEnv, player_names: list[str] | None = None) -> None:
        ...
        # Use player names from parameter or default names.
        if player_names is None:
            self.player_names: list[str] = [f"Player {i+1}" for i in range(self.env.num_players)]
        else:
            self.player_names = player_names

        # Initial board extents will be recalculated in update_board_extents().
        self.min_x: int = 0
        self.max_x: int = 0
        self.min_y: int = 0
        self.max_y: int = 0
        ...
        # Currently selected card (and its index in the current hand).
    ...
    def update_board_extents(self) -> None:
        """
        Update the board extents based on the placed cards.
        Always leave one extra cell (card) margin around the placed cards.
        Also update the canvas size accordingly.
        """
        if self.env.board:
    ...
    def transform(self, pos: tuple[int, int]) -> tuple[int, int]:
        ...
        """
        x, y = pos
        pixel_x: int = GUI_CONFIG['card_margin'] + (x - self.min_x) * (GUI_CONFIG['card_width'] + GUI_CONFIG['card_margin'])
        pixel_y: int = GUI_CONFIG['card_margin'] + (y - self.min_y) * (GUI_CONFIG['card_height'] + GUI_CONFIG['card_margin'])
    ...
    def inverse_transform(self, pixel: tuple[int, int]) -> tuple[int, int]:
        ...
        board_x: int = (x_pixel - GUI_CONFIG['card_margin']) // (GUI_CONFIG['card_width'] + GUI_CONFIG['card_margin']) + self.min_x
        board_y: int = (y_pixel - GUI_CONFIG['card_margin']) // (GUI_CONFIG['card_height'] + GUI_CONFIG['card_margin']) + self.min_y
        return board_x, board_y

    def draw(self) -> None:
        """
        Redraw the entire canvas including the board, current player info, and hand or play-again button.
        """
    ...
    def draw_board(self) -> None:
        ...
        for pos, card in self.env.board.items():
            ...
            card_canvas: tk.Canvas = draw_card(
                ...
                click_callback=lambda event, c=card: None  # Board cards need not be clickable here.
            )
    ...
    def draw_hand(self) -> None:
        ...
        # Draw each card in the current player's hand.
    ...
    def on_card_click(self, event: tk.Event, card: Card, index: int) -> None:
        """
        Handle click events on a card in the hand.
        If a goal card is hidden, flip it temporarily.
        If the card is already selected, rotate it.
        ...
        if card.type == 'goal':
            if card.hidden:
                card.hidden = False
                self.draw()
                self.root.after(2000, lambda: self.on_card_click(event, card, index))
            else:
                card.hidden = True
                self.draw()
            return

        # Toggle rotation if the same card is clicked twice.
        if self.selected_card is not None and self.selected_card == card:
            card.rotate()
        else:
            # Deselect all cards in the hand.
    ...
    def on_click(self, event: tk.Event) -> None:
        """
        Handle clicks on the board (background).
        If a card is selected, attempt to place it at the clicked board position via env.step.
        """
        # Do not process board clicks if the game is over.
        if self.env.done:
            return
        ...
        if self.selected_card is not None and self.selected_card_index is not None:
            ...
            if reward < 0:
                print("Invalid move!")
            # Clear selection and update the display.
